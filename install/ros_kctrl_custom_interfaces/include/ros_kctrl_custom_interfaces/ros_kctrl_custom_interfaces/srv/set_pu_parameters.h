// generated from rosidl_generator_c/resource/idl.h.em
// with input from ros_kctrl_custom_interfaces:srv/SetPUParameters.idl
// generated code does not contain a copyright notice

#ifndef ROS_KCTRL_CUSTOM_INTERFACES__SRV__SET_PU_PARAMETERS_H_
#define ROS_KCTRL_CUSTOM_INTERFACES__SRV__SET_PU_PARAMETERS_H_

#include "ros_kctrl_custom_interfaces/srv/detail/set_pu_parameters__struct.h"
#include "ros_kctrl_custom_interfaces/srv/detail/set_pu_parameters__functions.h"
#include "ros_kctrl_custom_interfaces/srv/detail/set_pu_parameters__type_support.h"

#endif  // ROS_KCTRL_CUSTOM_INTERFACES__SRV__SET_PU_PARAMETERS_H_
